defmodule Database do
  ...
  def change_password(username, old_password, new_password) when is_bitstring(username) and is_bitstring(old_password) and is_bitstring(new_password) do
    GenServer.call(__MODULE__, {:change_password, username, old_password, new_password})
  end

  # Server side

  def handle_call(:show, _from, {_, data} = state) do
    {:reply,{:ok, data}, state}
  end

  def handle_call({:save_user, input},_from, {_, data} = state) do
    data
    |> Enum.all?(fn user -> not User.compare(user, input) end)
    |> add(input, state)
  end

  def handle_call({:login, username, password},_from,{false, data} = state) do
    data
    |> Map.get(username)
  ...
  def handle_call({:login, _, _},_from,{true, _} = state) do
    {:reply, {:error, "A user is already logged in!"}, state}
  end

  def handle_call({:logout, username},_from,{true, data} = state) do
    data
    |> Map.get(username)
    |> logOut(state)
  end
  
  def handle_call({:logout, _},_from,{false, _} = state) do
    {:reply, {:error, "Noone is logged in!"}, state}
  end

  def handle_call({:change_password, username, old_password, new_password},_from,{true, data} = state) do
    data
    |> Map.get(username)
  ...
  def handle_call({:change_password, _, _, _},_from,{false, _} = state) do
    {:reply, {:error, "Log in to change your password!"}, state}
  end

  def handle_cast({:delete_user, name}, {isLoggedIn, data}) do
    new_data = Map.delete(data, name)
    {:noreply, {isLoggedIn, new_data}}
  end
  # Private functions

  defp add(true, %User{username: name} = u, {state, data} ) do
    new_data = Map.put(data, name, u)
    {:reply, {:ok, "User added"}, {state, new_data}}
  end
  ...
  defp logIn(%User{username: name} = u, {_, data}) do
    new_data = Map.put(data, name, %User{u | is_logged_in: true})
    {:reply, {:ok, "Logged in"} , {true, new_data}}
  end
  ...
  defp logOut(%User{username: name, is_logged_in: true} = u, {_, data}) do
    new_data = Map.put(data, name, %User{u | is_logged_in: false})
    {:reply, {:ok, "Logged out"} , {false, new_data}}
  end
  ...
  defp changePass(%User{username: name} = u, new_pass, { isLoggedIn, data}) do
    new_data = Map.put(data, name, %User{u | password: new_pass})
    {:reply, {:ok, "Password changed"} , {isLoggedIn, new_data}}
  end
